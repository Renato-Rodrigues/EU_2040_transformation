...
electricity_share_in_FE <- data.frame(
  electricity_share_in_FE = c("2040"),
  reference_value=c(
...
g[["Electricity Demand"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=0.3, colour="black", alpha=0.9, linewidth=1) +   
  geom_boxplot(outlier.shape = NA, width=0.6) +
  geom_hline(yintercept=2641, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Electricity\nDemand", y = 2641, label = "2022", angle=90, vjust = -0.5, hjust = 1.2, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["Targets"]] <- gridExtra::arrangeGrob(
  ...
  g[["Novel CDR"]] + theme(plot.margin=margin(b=20, l = 27)),
