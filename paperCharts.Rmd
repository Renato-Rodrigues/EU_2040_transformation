...
Generation <- data.frame(
  Electricity_Generation=c("coal_in_generation_(%)","gas_in_generation_(%)","wind_and_Solar_gen_(%)","wind_and_Solar_cap_(TW)"),
  reference_value=c(
...
plotData <- rbind(
  ...
  ,
  df %>% 
    filter(region == "EU27", period %in% c(2035:2050), tgt2030 %in% tgt2030Scen) %>%
    calc_addVariable( "`Emissions with LULUCF and with international transport`" = "`Emi|GHG|LULUCF national accounting` + `Emi|CO2|Energy|Demand|Transport|International Bunkers`" , units = "Mt CO2/yr", only.new=T) %>% 
...
plotData[plotData$value <= 0,]$value <- 0
...
data <-
  ...
  arrange(match(variable, c("25pct",vars))) %>%
  group_by(period) %>%
...
data[data$period!=2020,]$labels <- NA
data[data$period != 2040,]$pct <- NA
data[data$variable == "25pct",]$pct <- NA


max <- data %>%
  filter(variable == tail(vars,1), complement == TRUE) %>% 
  group_by(period)

data$labelsPosY <- data$ymid
data[data$labels %in% c("Solids"),]$labelsPosY <- 2015
data[data$labels %in% c("Bunkers","Industrial Processes","Liquids"),]$labelsPosY <- 2016
data[data$labels %in% c("Hydrogen"),]$labelsPosY <- 2010

data$pctPosY <- data$ymid
#data[data$Variable %in% c("Emi|GHG|Industrial Processes","Emi|CO2|Gross|Energy|Supply|Gases","Emi|GHG|Gross|Energy|Supply|Electricity"),]$pctPosY <- 2047
data[data$variable %in% c("Emi|CO2|Gross|Energy|Supply|Solids","Emi|CO2|Gross|Energy|Supply|Heat","Emi|CO2|Gross|Energy|Supply|Hydrogen"),]$pctPosY <- 2100
data[data$variable %in% c("Emi|CO2|Gross|Energy|Supply|Liquids"),]$pctPosY <- 2043
data$pctPosX <- data$xmid
#data[data$variable %in% c("Emi|GHG|Industrial Processes"),]$pctPosX <- data[data$variable %in% c("Emi|GHG|Industrial Processes"),]$pctPosX - 100
#data[data$variable %in% c("Emi|GHG|Gross|Energy|Demand|Industry"),]$pctPosX <- data[data$variable %in% c("Emi|GHG|Gross|Energy|Demand|Industry"),]$pctPosX + 100

#data$ext_Var <- as.character(data$ext_Var)

#remove labels for small values
data <- data %>% mutate(pct = ifelse(variable %in% vars[c("Hydrogen","Solids","Heat")], NA, pct))

g[["2040 Emissions - Energy and Industrial Processes - horizontal bar"]] <- data %>%
  ggplot(aes(xmid, ymid, fill = ext_Var)) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
                alpha = factor(period), color = variable)) +
  #geom_text(data=max, aes(label = period, y=period, x=xmax), vjust=0.5, hjust=-0.5) + 
  geom_text(aes(y = 2015, label = labels, group = labels, hjust=1, angle = 90), vjust=0.5, hjust=1.1, na.rm=TRUE, size = geom.text.single.size, angle = 90) +
  #geom_label_repel(aes(y = 2014, label = labels, group = labels), vjust=0.5, hjust=1, na.rm=TRUE, size = geom.text.single.size, angle = 90) +
  #geomtextpath::geom_textpath(aes(y = 2014, label = labels, group = labels, vjust=0.5, hjust=1, angle = 90),text_only = TRUE, na.rm=TRUE, size = geom.text.double.size) +
  geom_text(aes(x=pctPosX , y = 2045, label = pct), vjust=0.5, hjust=-0.1, na.rm=TRUE, size = geom.text.single.size, angle = 90) +
  #geomtextpath::geom_textpath(aes(x=pctPosX , y = 2046, label = pct),text_only = TRUE, na.rm=TRUE, size = geom.text.double.size) +
  #geom_text(aes(y = ymid + 3, label = labels, group = labels)) +
  scale_fill_manual(values = c(setNames(plotColor[names(vars)],vars),setNames(rep("transparent",length(vars)),paste0(vars,"_"))), labels = setNames(names(vars), vars)) +
  ...
  #scale_y_continuous(limits = c(1990, 2045)) +
  #scale_y_reverse(limits = c(2065, 2013)) +
  theme_doubleColumn() +
  theme(legend.position = "none") +
  scale_y_continuous(breaks=c(2020,2030,2040), minor_breaks = NULL, limits=c(2008,2047)) +
  ...
  coord_cartesian(clip="off") +
  theme(plot.margin = unit(c(1,0,2,0), "cm"))
  


ggsave(paste0(outFolder,"/2040 Emissions - Energy and Industrial Processes - horizontal bar.svg"), g[["2040 Emissions - Energy and Industrial Processes - horizontal bar"]] , device="svg", width = 12, height = 3, dpi=100, units = "in")
ggsave(paste0(outFolder,"/2040 Emissions - Energy and Industrial Processes - horizontal bar.png"), g[["2040 Emissions - Energy and Industrial Processes - horizontal bar"]] , device="png", width = 12, height = 3, dpi=300, units = "in")
...
g[["FE Gases"]] <- 
  ...
  geom_col(data=plotData %>% filter(scenario == mainScen), aes(fill = variable)) +
...
plotData <- df %>% 
  filter(region == "EU27", period  %in% c(2020,2030,2040,2050,2060), scenario == mainScen, variable %in% unlist(vars))  
...
g[["security of supply - fossil"]] <- ggplot(plotData %>% filter(period <= 2050, carrier != "Biomass"),aes(x=period,y=value)) +
  geom_bar(aes(fill=type),stat='identity') +
  geom_errorbar(data  = plotDataTotal %>% filter(period <= 2050, carrier != "Biomass"), aes(y=value, ymax=value, ymin=value, color=type), size=1) +
  facet_wrap(~carrier,nrow=1) +
   theme_doubleColumn() +
   theme(axis.title.y=element_blank()) +
   scale_fill_manual(values = plotColor, guide = guide_legend(nrow = 1)) + #, guide = guide_legend(nrow = 2) setNames(options$aesthetics$legend$color[names(vars)],vars)
...
plotData <- df %>% 
 ...
 g[["Carbon Management"]] <- ggplot(plotData,aes(x=CDRGroup,y=value)) +
  ...
  geom_bar(data=plotData %>% filter(scenario == mainScen), aes(fill=variable),stat='identity') + 
  geom_errorbar(data=plotData %>% filter(variable == vars[1], scenario == mainScen), aes(ymin=ribbon.min.total, ymax=ribbon.max.total), width=0.4, colour="black", alpha=0.9, size=1,position = position_dodge(width=2)) +
 ...
 ggsave(paste0(outFolder,"/Carbon Management.svg"), g[["Carbon Management"]] , device="svg", width = 6, height = 5, dpi=100, units = "in")
 ggsave(paste0(outFolder,"/Carbon Management.png"), g[["Carbon Management"]] , device="png", width = 6, height = 5, dpi=300, units = "in")
...
g[["Energy savings"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Energy\nsavings", y = 750, label = "FitFor55", angle=90, vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=787, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Energy\nsavings", y = 787, label = "REPowerEU", angle=90, vjust = 1.5, hjust = 0.7, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["Renewables share"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Renewables\nshare", y = 0.4, label = "RED II revised", angle=90, vjust = -0.5, hjust = 0.5, color = "black", alpha = 0.5) +
  geom_hline(yintercept=0.45, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Renewables\nshare", y = 0.45, label = "REPowerEU", angle=90, vjust = 1.5, hjust = 0.6, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["Targets"]] <- gridExtra::arrangeGrob(
  ...
  g[["Energy savings"]] + theme(plot.margin=margin(b=20, l = 57)),
  ...
  g[["Novel CDR"]] + theme(plot.margin=margin(b=20, l = 25)),
