...
indirect_electrification <- data.frame(
  indirect_electrification_EJ = c("total","buildings","transport","industry"),
  reference_value=c(
...
BEVs_in_LDV_data <- left_join(
  df %>% 
  filter(region == "EU27", period == 2040, variable %in% c("Stock|Transport|LDV|BEV"), tgt2030 %in% tgt2030Scen) %>%
  group_by(scenario) %>%    
  summarize(value = sum(value),.groups = "keep")
  ,
  df %>% 
  filter(region == "EU27", period == 2040, variable %in% c("Stock|Transport|LDV|BEV","Stock|Transport|LDV|FCEV","Stock|Transport|LDV|Hybrid Electric","Stock|Transport|LDV|Liquids","Stock|Transport|LDV|NG"), tgt2030 %in% tgt2030Scen) %>%
  group_by(scenario) %>%  
  summarize(total = sum(value)) %>% 
  select(scenario,total)  
  ,
...
rate2021_2030RenewablesShare <- 
  df %>%
    filter(region == "EU27", period == 2030, variable == "SE|Renewables Share", tgt2030 %in% tgt2030Scen) %>%
    mutate(numerator=value) %>%
    select(scenario,numerator) %>%
    mutate(denominator=addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value)) %>%
    mutate(percentage = numerator/denominator,
           rate = percentage ^ (1/9))

renewablesShare <- data.frame(
  result=c("Renewables_Share_upscaling_rates_2021_to_2030_%","Renewables_Share_upscaling_rates_2030_to_2040_%"),
  reference_value=c(
    round((rate2021_2030RenewablesShare %>% filter(scenario == mainScen) %>% pull(rate)-1)*100,1),
    round((rate2030_2040RenewablesShare %>% filter(scenario == mainScen) %>% pull(rate)-1)*100,1)
  ),
  min = c(
    round((min(rate2021_2030RenewablesShare %>% pull(rate))-1)*100,1),
    round((min(rate2030_2040RenewablesShare %>% pull(rate))-1)*100,1)
  ),
  max = c(
    round((max(rate2021_2030RenewablesShare %>% pull(rate))-1)*100,1),
...
#estimation of historical share in 2021 for FE electricity / FE with non-energy use and with bunkers
h <- (
      ...
      addHist$FE_consumption %>% mutate(value = value/1000000) %>% filter(period==2021) %>% pull(value) + # FE|w/o Non-energy Use w/o Bunkers - EJ
...
ElectricityShare <- data.frame(
  result=c("Electricity_Share_upscaling_rates_2021_to_2030_%","Electricity_Share_upscaling_rates_2030_to_2040_%","Transport_Electricity_Share_upscaling_rates_2030_to_2040_%","Buildings_Electricity_Share_upscaling_rates_2030_to_2040_%","Industry_Electricity_Share_upscaling_rates_2030_to_2040_%"),
  reference_value=c(
    round((rate2021_2030ElectricityShare %>% filter(scenario == mainScen) %>% pull(rate)-1)*100,1),
  ...
  min = c(
    round((min(rate2021_2030ElectricityShare %>% pull(rate))-1)*100,1),
  ...
  max = c(
    round((max(rate2021_2030ElectricityShare %>% pull(rate))-1)*100,1),
...
g[["GHG emi"]] <- ggplot(plotData, aes(x=period,y=value)) +
    ...
    geom_label_repel(data=data.frame(period=c(2042),value=(((2050-2042)/20)*(emi1990*(1-0.57)))), label = "linear reduction",
                     nudge_x = 10,
                     nudge_y = 800,
                     segment.square = TRUE,
                     point.padding = 0.6,
...
ggsave(paste0(outFolder,"/Primary Energy.svg"), g[["Primary Energy"]] , device="svg", width = 6, height = 5, dpi=100, units = "in")
ggsave(paste0(outFolder,"/Primary Energy.png"), g[["Primary Energy"]] , device="png", width = 6, height = 5, dpi=300, units = "in")
...
ggsave(paste0(outFolder,"/FE Gases - alt axis.svg"), g[["FE Gases - alt axis"]] , device="svg", width = 6, height = 4, dpi=100, units = "in")
ggsave(paste0(outFolder,"/FE Gases - alt axis.png"), g[["FE Gases - alt axis"]] , device="png", width = 6, height = 4, dpi=300, units = "in")
...
#   filter(region == "EU27", period %in% c(2020,2030,2040,2050), tgt2030 %in% tgt2030Scen, variable %in% unlist(vars), tgt2050 != "Npi") %>% 
#   mutate(variable = factor(variable, levels=vars))  %>% 
#   group_by(model,scenario,region,period,bioLim) %>% 
#   mutate(cumsum = cumsum(value),
#          sum = sum(value)) %>% 
#   group_by(model,region,variable,period,bioLim) %>% 
#   mutate(ribbon.max=max(cumsum),
#          ribbon.min=min(cumsum)) %>% 
#   group_by(model,region,period) %>% 
#   mutate(ribbon.max.total=max(sum),
...
# g[["FE Gases"]] <- 
#   ggplot(plotData,aes(x=period,y=value)) +
#   geom_col(data=plotData %>% filter(scenario %in% c(mainScen,"Nzero_55_bio12")), aes(fill = variable, alpha=factor(period))) +
#   #geom_errorbar(data=plotData %>% filter(period != 2020), aes(ymin=ribbon.min, ymax=ribbon.max, group=variable), width=8, colour="black", alpha=0.9, size=1,position = position_dodge(width=2)) +
#   geom_errorbar(data=plotData %>% filter(period != 2020, variable == vars[1], scenario %in%  c(mainScen,"Nzero_55_bio12")), aes(ymin=ribbon.min.total, ymax=ribbon.max.total), width=4, colour="black", alpha=0.9, linewidth=1,position = position_dodge(width=2)) +
#   # scale_y_continuous(labels = scales::percent, breaks = c(0,1,plotData[plotData$period == "2030",]$value,plotData[plotData$period == "2040",]$value,plotData[plotData$period == "2050",]$value)) +
#   coord_flip() +
#   facet_grid(bioLim~.) +
#   theme_doubleColumn() +
...
plotData$carrier <- factor(plotData$carrier, levels=c("Oil","Gas","Coal","Biomass"))
...
g[["security of supply - net fossil"]] <- ggplot(plotDataTotal %>% filter(period <= 2050, carrier != "Biomass"),aes(x=period,y=value)) +
  geom_hline(yintercept=0, color = "black", linewidth=2, alpha = 0.5) +
  geom_bar(aes(fill=as.factor(period)),stat='identity', alpha = 0.7) +
  facet_wrap(~carrier,nrow=1) +
  theme_doubleColumn()  +
  theme(axis.title.y=element_blank()) +
...
ggsave(paste0(outFolder,"/security of supply - net fossil.svg"), g[["security of supply - net fossil"]] , device="svg", width = 6, height = 4.5, dpi=100, units = "in")
ggsave(paste0(outFolder,"/security of supply - net fossil.png"), g[["security of supply - net fossil"]] , device="png", width = 6, height = 4.5, dpi=300, units = "in")
...
```{r, echo=FALSE, fig.width=6, fig.height=5}
  ...
  g[["security of supply - net fossil"]]
...
ggsave(paste0(outFolder,"/Hydrogen - alt axis.svg"), g[["Hydrogen - alt axis"]] , device="svg", width = 6, height = 4, dpi=100, units = "in")
ggsave(paste0(outFolder,"/Hydrogen - alt axis.png"), g[["Hydrogen - alt axis"]] , device="png", width = 6, height = 4, dpi=300, units = "in")
...
salestot <- df %>% 
  filter(region == "EU27", period >= 2020, period <= 2060, scenario == mainScen, variable == "Sales|Transport|LDV") %>% 
  group_by(period, value) %>%
  summarize(.groups = "keep") %>%
  rename(tot = value)

plotData <- 
  inner_join(plotData, salestot, by = "period") %>%
  mutate(value = value/tot, by = "period")
...
totstockData <- df %>% 
  filter(region == "EU27", period >= 2020, period <= 2060, scenario == mainScen, variable == "Stock|Transport|LDV") %>% 
  group_by(period, value) %>%
  summarize(.groups = "keep") %>%
  rename(tot = value)

stockData <- 
  inner_join(stockData,totstockData, by = "period") %>% 
  summarize(value = value/tot) %>%
  mutate(variable = "ICE Stock")
...
scale_y_continuous(limits = c(0, 1.1), breaks=c(0,1), labels = scales::percent, name = "Sales") +
  ...
  # scale_y_continuous(limits = c(0, 1.1), breaks=c(0,1), labels = scales::percent, name = "Sales", # - Million vehicles", 
 #                     #breaks = c(round(emi1990), round(emi1990*0.6), round(emi1990*0.45), round(emi1990*0.3), round(emi1990*0.15), 0),
 #                     sec.axis = sec_axis( trans=~.*1, breaks=c(0,1), name="Stock", labels = scales::percent)) +
 #                                          #, labels = scales::percent, breaks = c(1,0.4,0.55,0.7,0.85, 0))) +
  theme(legend.text=element_text(size=theme.double.text.size),
...
ggsave(paste0(outFolder,"/LDV Sales.svg"), g[["LDV Sales"]] , device="svg", width = 6, height = 4.5, dpi=100, units = "in")
ggsave(paste0(outFolder,"/LDV Sales.png"), g[["LDV Sales"]] , device="png", width = 6, height = 4.5, dpi=300, units = "in")
...
ggsave(paste0(outFolder,"/Industry Emissions.svg"), g[["Industry Emissions"]] , device="svg", width = 6, height = 5.5, dpi=100, units = "in")
ggsave(paste0(outFolder,"/Industry Emissions.png"), g[["Industry Emissions"]] , device="png", width = 6, height = 5.5, dpi=300, units = "in")
...
g[["Buildings Heating - Useful energy - Electricity Based - alt axis"]] <- 
  ggplot(plotData,aes(x=period,y=value)) +
  geom_col(data=plotData %>% filter(scenario == mainScen), aes(fill = variable, alpha=factor(period))) +
  geom_errorbar(data=plotData %>% filter(period != 2020, variable == vars[1], scenario == mainScen), aes(ymin=ribbon.min.total, ymax=ribbon.max.total), width=4, colour="black", alpha=0.9, size=1,position = position_dodge(width=2)) +
  # scale_y_continuous(labels = scales::percent, breaks = c(0,1,plotData[plotData$period == "2030",]$value,plotData[plotData$period == "2040",]$value,plotData[plotData$period == "2050",]$value)) +
...
ggsave(paste0(outFolder,"/Buildings Heating - Useful energy - Electricity Based - alt axis.svg"), g[["Buildings Heating - Useful energy - Electricity Based - alt axis"]] , device="svg", width = 6, height = 4, dpi=100, units = "in")
ggsave(paste0(outFolder,"/Buildings Heating - Useful energy - Electricity Based - alt axis.png"), g[["Buildings Heating - Useful energy - Electricity Based - alt axis"]] , device="png", width = 6, height = 4, dpi=300, units = "in")
...
plotData <- df %>% 
 ...
 ggsave(paste0(outFolder,"/Carbon Management.svg"), g[["Carbon Management"]] , device="svg", width = 6, height = 5.5, dpi=100, units = "in")
 ggsave(paste0(outFolder,"/Carbon Management.png"), g[["Carbon Management"]] , device="png", width = 6, height = 5.5, dpi=300, units = "in")
...
h <- refEmi %>% filter(variable == "Emissions with LULUCF and with international transport", region == "EU27", pollutant == "All greenhouse gases - (CO2 equivalent)") %>% ungroup() %>% select(region,variable,period,value)
emi1990 <- tgt %>% filter(target == "Emissions with LULUCF and with international transport") %>% pull(reference)
reduction_h <- 1- (h %>% filter(period==2021) %>% pull(value) / emi1990)
...
g[["Emissions reductions"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Emissions\nreductions", y = reduction_h, label = "2021", angle=90, vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
...
g[["milestone - vert"]][["Emissions reductions"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_hline(yintercept=0.55, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 0.55, label = "FitFor55", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=reduction_h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = reduction_h, label = "2021", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
...
g[["milestone - hor"]][["Emissions reductions"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=0.55, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 0.55, label = "FitFor55", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  geom_hline(yintercept=reduction_h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = reduction_h, label = "2021", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
  ...
  annotate("text", x = -Inf, y = -Inf, label = "(%)", hjust = 1, color = "#666666", size=0.8*geom.text.double.size) 
...
g[["Energy savings"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Final Energy\nDemand", y = 787, label = "FitFor55", angle=90, vjust = 1.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=787, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Final Energy\nDemand", y = 750, label = "REPowerEU", angle=90, vjust = -0.5, hjust = 0.7, color = "black", alpha = 0.5) +
  geom_hline(yintercept=h %>% filter(period==2021) %>% pull(value), color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Final Energy\nDemand", y = h %>% filter(period==2021) %>% pull(value), label = "2021", angle=90, vjust = -0.5, hjust = 0.7, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - vert"]][["Energy savings"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_hline(yintercept=750, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  geom_hline(yintercept=787, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 750, label = "REPowerEU", vjust = 1.5, hjust = 1, color = "black", alpha = 0.5) +
  annotate("text", x = Inf, y = 787, label = "FitFor55", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=h %>% filter(period==2021) %>% pull(value), color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h %>% filter(period==2021) %>% pull(value), label = "2021", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
  ...
  theme(strip.text = element_text(angle = 90)) +
  coord_cartesian(clip = 'off') +
  annotate("text", x = -Inf, y = Inf, label = "(Mtoe)", hjust = 1, color = "black", alpha = 0.5) +
  theme(axis.text.x = element_text(size = 1*theme.double.text.size)) +
  theme(axis.text.y = element_text(size = 1*theme.double.text.size))

g[["milestone - hor"]][["Energy savings"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=750, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  geom_hline(yintercept=787, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 750, label = "REPowerEU", angle=90, vjust = -1, hjust = 0, color = "black", alpha = 0.5) +
  annotate("text", x = Inf, y = 787, label = "FitFor55", angle=90, vjust = 1, hjust = 0, color = "black", alpha = 0.5) +
  geom_hline(yintercept=h %>% filter(period==2021) %>% pull(value), color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h %>% filter(period==2021) %>% pull(value), label = "2021", angle=90, vjust = -1, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
  ...
  theme(axis.text.y = element_text(size = 1*theme.double.text.size)) +
  theme(strip.text.x = element_text(hjust = 0, margin=margin(l=0, b=20))) +
  annotate("text", x = -Inf, y = -Inf, label = "(Mtoe)", hjust = 1, color = "#666666", size=0.8*geom.text.double.size)
...
g[["Electricity Demand"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Electricity\nDemand", y = h, label = "2022", angle=90, vjust = -0.5, hjust = 1.2, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
  ...
  theme(axis.text.x = element_text(size = 0.8*theme.double.text.size)) +
  theme(axis.text.y = element_text(size = 1.2*theme.double.text.size)) +
  geom_text(data=data.frame(period=c(2030,2040),label=c("(TWh)",NA)), aes(label = label,x = 0, y = 0), color = "#666666", size=0.8*geom.text.double.size, hjust=1.2, na.rm = TRUE)

g[["milestone - vert"]][["Electricity Demand"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h, label = "2022", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
  ...
  theme(strip.text = element_text(angle = 90)) +
  coord_cartesian(clip = 'off') +
  annotate("text", x = -Inf, y = Inf, label = "(TWh)", hjust = 1, color = "black", alpha = 0.5) +
  theme(axis.text.x = element_text(size = 1*theme.double.text.size)) +
  theme(axis.text.y = element_text(size = 1*theme.double.text.size))

g[["milestone - hor"]][["Electricity Demand"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h, label = "2022", angle=90, vjust = -1, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
  ...
  theme(axis.text.y = element_text(size = 1*theme.double.text.size)) +
  theme(strip.text.x = element_text(hjust = 0, margin=margin(l=0, b=20))) +
  annotate("text", x = -Inf, y = -Inf, label = "(TWh)", hjust = 1, color = "#666666", size=0.8*geom.text.double.size)
...
g[["Renewables Share"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  geom_hline(yintercept=addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value), color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Renewables\nShare in\nFinal Energy", y = addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value), label = "2021", angle=90, vjust = -0.5, hjust = 0.6, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - vert"]][["Renewables Share"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=0.4, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 0.4, label = "RED II revised", vjust = 1.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=0.45, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 0.45, label = "REPowerEU", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value), color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value), label = "2021", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
...
g[["milestone - hor"]][["Renewables Share"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=0.4, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 0.4, label = "RED II revised", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  geom_hline(yintercept=0.45, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 0.45, label = "REPowerEU", angle=90, vjust = 1.5, hjust = 0, color = "black", alpha = 0.5) +
  geom_hline(yintercept=addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value), color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = addHist$res %>% filter(type == "RES", period == 2021) %>% pull(value), label = "2021", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["Wind capacity"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Wind\ncapacity", y = 255, label = "2022", angle=90, vjust = -0.5, hjust = 1.2, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - vert"]][["Wind capacity"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=255, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 255, label = "2022", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
  ...
  theme(strip.text = element_text(angle = 90)) +
  coord_cartesian(clip = 'off') +
  annotate("text", x = -Inf, y = Inf, label = "(GW)", hjust = 1, color = "black", alpha = 0.5) +
  theme(axis.text.x = element_text(size = 1*theme.double.text.size)) +
  theme(axis.text.y = element_text(size = 1*theme.double.text.size))

g[["milestone - hor"]][["Wind capacity"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=255, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 255, label = "2022", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
  ...
  theme(axis.text.y = element_text(size = 1*theme.double.text.size)) +
  theme(strip.text.x = element_text(hjust = 0, margin=margin(l=0, b=20))) +
  annotate("text", x = -Inf, y = -Inf, label = "(GW)", hjust = 1, color = "#666666", size=0.8*geom.text.double.size)
...
g[["Solar photovoltaics"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Solar\nphotovoltaics", y = 208.9, label = "2022", angle=90, vjust = -0.5, hjust = 1.2, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - vert"]][["Solar photovoltaics"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=600, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 600, label = "REPowerEU", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  geom_hline(yintercept=208.9, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 208.9, label = "2022", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
...
g[["milestone - hor"]][["Solar photovoltaics"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=600, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 600, label = "REPowerEU", angle=90, vjust = 1.5, hjust = 0, color = "black", alpha = 0.5) +
  geom_hline(yintercept=208.9, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 208.9, label = "2022", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - vert"]][["Carbon Capture and Storage"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=50, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 50, label = "Net-Zero\nIndustry Act", vjust = 0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
  ...
  theme(strip.text = element_text(angle = 90)) +
  coord_cartesian(clip = 'off') +
  annotate("text", x = -Inf, y = Inf, label = "(Mt CO2/yr)", hjust = 1, color = "black", alpha = 0.5) +
  theme(axis.text.x = element_text(size = 1*theme.double.text.size)) +
  theme(axis.text.y = element_text(size = 1*theme.double.text.size))

g[["milestone - hor"]][["Carbon Capture and Storage"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=50, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = 50, label = "Net-Zero\nIndustry Act", angle=90, vjust = 0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
  ...
  theme(axis.text.y = element_text(size = 1*theme.double.text.size)) +
  theme(strip.text.x = element_text(hjust = 0, margin=margin(l=0, b=20))) +
  annotate("text", x = -Inf, y = -Inf, label = "(Mt CO2/yr)", hjust = 1, color = "#666666", size=0.8*geom.text.double.size)
...
g[["milestone - hor"]][["Novel CDR"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
...
g[["Direct Electrification"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=0.6) +
        ...
        #       #position=position_nudge(x=c(0.1,0.35,0.35,0.35,0.1)), 
         #      hjust=-1, angle = 90) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Electricity\nShare in\nFinal Energy", y = h, label = "2021", angle=90, vjust = -0.5, hjust = 1.2, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - vert"]][["Direct Electrification"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h, label = "2021", vjust = -0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
...
g[["milestone - hor"]][["Direct Electrification"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h, label = "2021", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["Carbon-free electricity share"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  geom_boxplot(outlier.shape = NA, width=0.6) +
        ...
        #       #position=position_nudge(x=c(0.1,0.35,0.35,0.35,0.1)), 
         #      hjust=-1, angle = 90) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = "Carbon-free\nelectricity\nshare", y = h, label = "2022", angle=90, vjust = -0.5, hjust = 1.2, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - hor"]][["Carbon-free electricity share"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=h, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = Inf, y = h, label = "2022", angle=90, vjust = -0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
g[["milestone - hor"]][["Residual Fossil Emissions"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
...
g[["Domestic Hydrogen Production"]] <- ggplot(plotData, aes(x = target, y = value, group=period)) +
  ...
  annotate("text", x = "Domestic\nHydrogen\nProduction", y = 10, label = "REPowerEU\ndomestic", angle=90, vjust = 0.5, hjust = 0.5, color = "black", alpha = 0.5) +
  facet_grid(target~period, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  ...
  theme(axis.text.x = element_text(size = 0.8*theme.double.text.size)) +
  theme(axis.text.y = element_text(size = 1.2*theme.double.text.size)) +
  geom_text(data=data.frame(period=c(2030,2040),label=c("(Mtoe)",NA)), aes(label = label,x = 0, y = 0), color = "#666666", size=0.8*geom.text.double.size, hjust=1.4, na.rm = TRUE)

g[["milestone - vert"]][["Domestic Hydrogen Production"]] <- ggplot(plotData, aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=10, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = -Inf, y = 10, label = "REPowerEU\ndomestic", vjust = 0.5, hjust = 0, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  theme(axis.title.y=element_blank()) +
  theme(strip.text.y = element_blank()) +
...
g[["milestone - hor"]][["Domestic Hydrogen Production"]] <- ggplot(plotData %>% mutate(target = gsub("\n", " ", target)), aes(x = period, y = value, group=period)) +
  geom_errorbar(aes(ymin=min, ymax=max), width=6, colour="black", alpha=0.9, linewidth=1) +
  geom_boxplot(outlier.shape = NA, width=6) +
  geom_hline(yintercept=10, color = "black", linewidth=1, alpha = 0.5, linetype = "dashed") +
  annotate("text", x = -Inf, y = 10, label = "REPowerEU\ndomestic", angle=90, vjust = 0.5, hjust = 1, color = "black", alpha = 0.5) +
  facet_grid(.~target, scales="free") +
  theme_doubleColumn() +
  coord_flip(clip = 'off') +
  theme(axis.title.y=element_blank()) +
...
#ggsave(paste0(outFolder,"/milestone - vert targets.svg"), g[["milestone - vert targets"]] , device="svg", width = 12, height = 17, dpi=100, units = "in")
#ggsave(paste0(outFolder,"/milestone - vert targets.png"), g[["milestone - vert targets"]] , device="png", width = 12, height = 17, dpi=300, units = "in")
...
ggsave(paste0(outFolder,"/milestone - hor targets.svg"), g[["milestone - hor targets"]] , device="svg", width = 12, height = 14, dpi=100, units = "in")
ggsave(paste0(outFolder,"/milestone - hor targets.png"), g[["milestone - hor targets"]] , device="png", width = 12, height = 14, dpi=300, units = "in")
