...
g[["target"]] <- ggplot(data=plotData,aes(x=period,y=reduction,fill=bar)) +
  ...
  geom_errorbar(aes(ymin=min, ymax=max), width=2, colour="black", alpha=0.9, linewidth=1.3) +
  geom_boxplot(data = plotData, 
               aes(x=period, y=reduction, group=period),
               fill = "white",
               outlier.shape = NA, 
               width=2) +
  geom_hline(yintercept=0, color = "black", linewidth=2, alpha = 0.5) +
...
boxData <- fulldata %>%
  ...
  left_join(
    plotData %>%
      filter(period == 2040) %>%
      select(variable, xmin),
    by = "variable"
  ) %>%
  mutate(
    x = xmin + net,   # shifted by main-scenario 2040 stack
    ymid = 2040
  )
...
g[["2040 Net Emissions - Energy and Industrial Processes - horizontal bar - net only"]] <- plotData %>%
  ...
  geom_errorbar(
    data = errData,
    inherit.aes = FALSE,
    aes(xmin = xmin, xmax = xmax, y = ymid),
    width = 4,
    colour = "black",
    linewidth = 0.8, 
    position = "identity"
  ...
  geom_text(data=plotData %>% filter(period == 2040), aes(x=xmin + (xmax-xmin)/2, y = 2045, label = pct1990), vjust=0.5, hjust=-0.1, na.rm=TRUE, size = geomText.size * 0.8, angle = 90) +
...
Figure 1. Greenhouse gas emissions for scenarios reaching climate neutrality by 2050. (a) Total greenhouse gas (GHG) emissions (Mt CO₂eq/yr). Dashed line shows historical emissions; solid line shows the reference scenario (57% reduction by 2030, reference final energy projections, biomass availability limited to 7.5 EJ/yr and default CCS assumption), with shading indicating the range across the full scenario ensemble. (b) GHG emission reductions relative to 1990 (%). Bars indicate reference-scenario values; error bars show the scenario range. Labels denote the minimum–maximum reduction across the ensemble. Reductions are based on GHG emissions including LULUCF and intra-EU aviation for 2030–2040, and including LULUCF and all bunkers for 2045–2050. (c) Sectoral breakdown of gross GHG emissions in the reference scenario (Mt CO₂/yr). (d) Sectoral breakdown of residual CO₂ emissions reduction per decade. Bars show residual emissions by sector relative to 1990 levels; percentages indicate the remaining share of sectoral emissions compared to 1990. Error bars indicate the scenario range. Box plots show the median (centre line), interquartile range (box, Q1–Q3), whiskers extending to minimum and maximum values.
...
Figure 2. Electricity supply and demand. (a) Electricity generation shares by technology in the reference scenario. (b) Upscaling of solar photovoltaics capacity in the reference scenario compared with REPowerEU targets; shaded areas indicate the range across the full scenario ensemble. (c) The transition to electricity as the main energy carrier, shown as the electricity share by sector.
...
label_map <- c("PE|Oil" = "Oil", 
               "PE|Gas" = "Gas", 
               "PE|Coal" = "Coal", 
               "PE|Biomass" = "Biomass")

# Create a subset for the labels at the year 2010
label_data <- h %>%
  filter(period == 2005,                   # Filter for the year 2010
         variable %in% names(label_map)) %>%
  mutate(clean_label = label_map[as.character(variable)]) # Add clean names
...
g[["Primary Energy"]] <- ggplot(plotData,aes(x=period,y=value)) +
    ...
    geom_text(data = label_data, 
              aes(y = value, label = clean_label, color = variable), 
              size = geomText.size*0.7,
              x = 2005,             # Force x position to 2010
              hjust = 0,            # Right align text so it ends at 2010
              vjust= c(1.2,1.5,-1,1.5),
              #nudge_x = -1,         # Shift slightly left to avoid overlapping the line start
              fontface = "bold", 
              #size = 4, 
              show.legend = FALSE) +
...
g[["FE Gases"]] <- 
  ...
  geom_boxplot(data = plotData %>% 
                 filter(period %in% c(2030, 2040, 2050), variable == vars[1]), 
               aes(x=period, y=sum, group=period),
               outlier.shape = NA, 
               width=2) +
  coord_flip() +
...
g[["security of supply - net fossil with range"]] <- 
  ...
  geom_errorbar(data=plotData %>% filter(period == 2040, scenario == mainScen), aes(ymin=ribbon.min, ymax=ribbon.max), width=4, colour="black", alpha=0.9, linewidth=1, position = "identity") +
  geom_boxplot(data = plotData %>% filter(period == 2040),
               outlier.shape = NA,
               width=4, position = "identity") +
...
g[["Hydrogen"]] <- 
  ...
  geom_boxplot(data = plotData %>% 
                 filter(period %in% c(2030, 2040, 2050), type == names(color)[1]), 
               aes(x=period, y=sum, group=period),
               outlier.shape = NA, 
               width=2) +
  coord_flip() +
...
Figure 3. Fuel Supply and Demand. (a) Primary Energy demand by fuel. (b) Net imports of fossil energy carriers. (c) Final energy demand for gaseous fuels. (d) Hydrogen production by supply type. Box plots show the median (centre line), interquartile range (box, Q1–Q3), whiskers extending to minimum and maximum values. 
...
g[["Buildings heating useful energy"]] <-
  ...
  geom_boxplot(data = plotData %>% 
                 filter(period %in% c(2030, 2040, 2050), name == "Electricity (resistence)"), 
               aes(x=period, y=sum, group=period),
               outlier.shape = NA, 
               width=3) +
  coord_flip() +
...
plotData <- df %>% 
 ...
 g[["Carbon Management"]] <- ggplot(plotData,aes(x=CDRGroup,y=value)) +
  ...
  geom_boxplot(data = plotData %>% 
               filter(period %in% c(2030, 2040, 2050), variable == vars[4]), 
             aes(x=CDRGroup, y=sum, group=period),
             outlier.shape = NA, 
             width=0.4) +
...
Figure 4. Sectoral transformation indicators. (a) Transport final energy; (b) Light duty vehicles sales shares (bars) and share of ICEs in total LDV stock (line); (c) Useful energy demand for heating in buildings; (d) Industry emissions; (e) Carbon Capture and Storage by activity (upper bar), origin (middle bar) and by destination (lower bar). Box plots show the median (centre line), interquartile range (box, Q1–Q3), whiskers extending to minimum and maximum values.
...
ggsave(paste0(outChartsFolder,"/svg/Figure 5. Key energy transformation indicators.svg"), g[["Figure 5"]] , device="svg", width = 12, height = 14, dpi=100, units = "in")
ggsave(paste0(outChartsFolder,"/png/Figure 5. Key energy transformation indicators.png"), g[["Figure 5"]] , device="png", width = 12, height = 14, dpi=300, units = "in")
...
Figure 5. Comparison of key energy transformation indicators including fit-for-55 policy targets for 2030 and 2040. Red cross symbols represent the results of the reference scenario. Box plots show the median (centre line), interquartile range (box, Q1–Q3), whiskers extending to minimum and maximum values of the sensitivity scenarios, while the shaded areas show the distribution of the scenario results. More information about the chart symbols and indicators can be found at the supplementary notes. 
...
plotData <- df %>%
  ...
  mutate(tgt2030 = factor(tgt2030, levels=labels[c("55","57","59")]),
         ...
         lim = factor(lim, levels=labels[c("default", "unlimCCS", "limCCS", "limH2", "limVRE", "limVRE_limInt", "limVRE_strgLimInt")])
         )
...
g[["Carbon Price - line"]] <- ggplot(plotData,aes(x=period,y=value,group = scenario)) +
  ...
  scale_linetype_manual(values = c("55%"="twodash", "57%"="solid", "59%"="dashed"), guide = guide_legend(title="Short-term policy", ncol =3, byrow = T, order = 1, override.aes = list(linewidth = 1))) +   
  scale_color_manual(values = c("reference"="#ae0000", "EED 2018"="#db4646", "FitFor55"="#0090ab", "RePowerEU"="#095786"), guide = guide_legend(title="Efficiency target", ncol =2, byrow = F, order = 2, override.aes = list(linewidth = 1.5))) + 
  scale_fill_manual(values = c("4 EJ"="#fedc97", "7.5 EJ"="#b5b682", "12 EJ"="#7c9885", "20 EJ"="#28666e"), guide = guide_legend(title="Biomass availability limited to", ncol =2, byrow = F, override.aes = list(shape = 21, size = 6, colour = "transparent"), order = 3)) +
  scale_shape_manual(values = c("default"=21, "unlimited CCS"=24, "limited CCS"=25,"limited H2"=23,"limited VRE"=22, "lim VRE&Integ."=12, "lim VRE&Int_alt"=14), 
                     breaks = c("default", "limited H2", "unlimited CCS", "limited CCS", "limited VRE", "lim VRE&Integ."),
                     guide = guide_legend(title="Technology evolution", ncol =2, byrow = T, order = 4, override.aes = list(size = 6))) +
...
labels <- c(
  ...
  "FitFor55 eff"="FitFor55", 
  "RePowerEU eff"="RePowerEU",
  "BioLim4"="limited to 4 EJ",
  "BioLim7.5"="limited to 7.5 EJ", 
  "BioLim12"="limited to 12 EJ", 
  "BioLim20"="limited to 20 EJ",
  "default"="default",
  "unlimCCS"="unlimited CCS",
  "limCCS"="limited CCS",
  "limH2"="limited H2",
  "limVRE"="limited VRE",
  "limVRE_limInt"="limited VRE&Integration_alt",
  "limVRE_strgLimInt"="lim VRE&Integ."
)
...
ggsave(paste0(outChartsFolder,"/png/Supplementary figure 1. EU-27 carbon price.png"), g[["Supplementary figure 1"]], device="png", width = 12, height = 11, dpi=300, units = "in")
ggsave(paste0(outChartsFolder,"/svg/Supplementary figure 1. EU-27 carbon price.svg"), g[["Supplementary figure 1"]], device="svg", width = 12, height = 11, dpi=100, units = "in")
...
vars <- c(
  "CO2 - Energy (including BECCS)" = "Emi|CO2|Energy",
...
plotColor <- c(
  "CO2 - Energy (including BECCS)" = "#3D4849",
...
g[["BECCS"]] <- ggplot(plotData,aes(x=period,y=value,group = scenario)) +
  ...
  scale_shape_manual(values = c("default"=21,"unlimCCS"=24,"limCCS"=25,"limH2"=23,"limVRE"=22,"limVRE_limInt"=12,"limVRE_strgLimInt"=14), guide = guide_legend(nrow =2, byrow = T, order = 4, override.aes = list(size = 8))) +
...
g[["DAC"]] <- ggplot(plotData,aes(x=period,y=value,group = scenario)) +
  ...
  scale_shape_manual(values = c("default"=21,"unlimCCS"=24,"limCCS"=25,"limH2"=23,"limVRE"=22,"limVRE_limInt"=12,"limVRE_strgLimInt"=14), 
                     breaks = c("default", "limH2", "unlimCCS", "limCCS", "limVRE", "limVRE_limInt", "limVRE_strgLimInt"),
                     guide = guide_legend(ncol =2, byrow = T, order = 4, override.aes = list(size = 6))) +
