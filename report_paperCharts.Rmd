...
#conclusion
# df %>% 
#     filter(region == "EU27", period %in% c(2040), tgt2030 %in% tgt2030Scen) %>%
#     calc_addVariable( "`Emissions with LULUCF and with international transport`" = "`Emi|GHG|LULUCF national accounting` + `Emi|CO2|Energy|Demand|Transport|International Bunkers`" , units = "Mt CO2/yr", only.new=T) %>% 
#     mutate(reduction = round(value / tgt %>% filter(target == "Emissions with LULUCF and with international transport") %>% pull(reference) ,2)) %>% 
#     select(-variable) %>%
#     filter(tgt2050 != "Npi") %>%
...
#            range=max-min,
#            labelRange = ifelse(round((1-max) * 100) == round((1-min) * 100), paste0(round((1-max) * 100), "%"), paste0(round((1-max) * 100), "% to ", round((1-min) * 100) ,"%"))) %>%
#   filter(scenario == mainScen, period == 2040) %>% pull(labelRange)
...
g[["2040 Emissions - Energy and Industrial Processes - horizontal bar"]] <- plotData %>% 
  ...
  #theme(plot.title = element_text(margin=margin(t=0,r=0,b=45,l=-200), hjust=1))
  labs(title = "(c) Sectoral breakdown of net CO2 emissions from energy and industrial processes (%)") + 
  theme(plot.title = element_text(margin=margin(t=0,r=0,b=45,l=0), hjust=1))
...
g[["Figure 1 - gross"]] <- gridExtra::arrangeGrob(
  g[["GHG emi"]],
  g[["target"]],
...
ggsave(paste0(outChartsFolder,"/png/Figure 1. Emission results panel (gross).png"), g[["Figure 1 - gross"]], device="png", width = 12, height = 8, dpi=300, units = "in")
ggsave(paste0(outChartsFolder,"/svg/Figure 1. Emission results panel (gross).svg"), g[["Figure 1 - gross"]], device="svg", width = 12, height = 8, dpi=100, units = "in")
...
<!-- (c) Breakdown of residual net CO2 emissions from energy and industrial processes (% in relation to 2020) -->

```{r chart_figure1c_gross, echo=FALSE}
...
if(!("Carbon Management|Storage|Pe2Se|Other" %in% unique(df$variable))){
  df <- rbind(df,df %>% calc_addVariable("`Carbon Management|Storage|Pe2Se|Other`" = "`Carbon Management|Storage|Biomass|Pe2Se|Gases w/ couple prod` + `Carbon Management|Storage|Biomass|Pe2Se|Hydrogen w/ couple prod` + `Carbon Management|Storage|Biomass|Pe2Se|Liquids w/ couple prod` + `Carbon Management|Storage|Fossil|Pe2Se|Gases w/ couple prod` + `Carbon Management|Storage|Fossil|Pe2Se|Hydrogen w/ couple prod` + `Carbon Management|Storage|Fossil|Pe2Se|Liquids w/ couple prod`", units = "Mt CO2/yr", only.new=T))
  df <- rbind(df,df %>% calc_addVariable("`Carbon Management|Storage|Pe2Se|Electricity`" = "`Carbon Management|Storage|Biomass|Pe2Se|Electricity w/ couple prod` + `Carbon Management|Storage|Fossil|Pe2Se|Electricity w/ couple prod`", units = "Mt CO2/yr", only.new=T))
  
  df <- rbind(df,df %>% calc_addVariable("`Carbon Management|Storage|Industry Extended`" = "`Carbon Management|Storage|Industry Energy` + `Carbon Management|Storage|Waste|Plastics Incineration`", units = "Mt CO2/yr", only.new=T))
}

varsCC <- list(
  "Industrial\nProcesses" = "Carbon Management|Storage|Industry Process",
  "Industry" = "Carbon Management|Storage|Industry Extended",
  "Other Energy\nConversion" = "Carbon Management|Storage|Pe2Se|Other",
  "Electricity" = "Carbon Management|Storage|Pe2Se|Electricity"
)
...
#       "Carbon Management|Storage|Biomass|Pe2Se|+|Electricity w/ couple prod",
#       "Carbon Management|Storage|Biomass|Pe2Se|+|Gases w/ couple prod",
#       "Carbon Management|Storage|Biomass|Pe2Se|+|Hydrogen w/ couple prod",
#       "Carbon Management|Storage|Biomass|Pe2Se|+|Liquids w/ couple prod",
#     "Carbon Management|Storage|+|Fossil|Pe2Se",
#       "Carbon Management|Storage|Fossil|Pe2Se|+|Electricity w/ couple prod",
#       "Carbon Management|Storage|Fossil|Pe2Se|+|Gases w/ couple prod",
#       "Carbon Management|Storage|Fossil|Pe2Se|+|Hydrogen w/ couple prod",
#       "Carbon Management|Storage|Fossil|Pe2Se|+|Liquids w/ couple prod",
#     "Carbon Management|Storage|+|Industry Energy",
#     "Carbon Management|Storage|+|Industry Process",
#     "Carbon Management|Storage|+|CDR energy",
#     "Carbon Management|Storage|+|DAC",
#     "Carbon Management|Storage|+|OAE calcination",
#     "Carbon Management|Storage|+|Waste|Plastics Incineration"
#   )
# 
# View(df %>%
#        filter(region == "EU27",
...
CCData <- df %>%
  ...
  mutate(CCData = -as.numeric(value),
         ...
         #variable = factor(variable, levels = varsCC)
         ) %>%
  select(-value)

#View(CCData %>% filter(scenario == mainScen))
...
plotData <- fulldata %>%
  ...
  mutate(complement = FALSE) %>%
  group_by(variable) %>%
  mutate(pct = paste0(round(net / net[period==2020],2)*100, "%")) %>%
  bind_rows(
    fulldata %>%
      filter(scenario == mainScen) %>%
      mutate(complement = TRUE) %>%
...
g[["2040 Net Emissions - Energy and Industrial Processes - horizontal bar"]] <- plotData %>% 
  ggplot(aes(xmid, ymid, fill = ext_Var)) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
                alpha = factor(period), color = variable)) +
  
  geom_rect_pattern(data = plotData %>% filter(CCData != 0), aes(xmin = xmax + CCData, xmax = xmax, ymin = ymin, ymax = ymax, alpha = factor(period), fill = ext_Var, color = variable, pattern_alpha = factor(period)), pattern_color = NA, pattern_fill = "white", pattern="stripe", pattern_spacing = 0.05, pattern_aspect_ratio = 1, pattern_density      = 0.2) +
  ...
  geom_text(data=plotData %>% filter(period == 2020), aes(y = 2015, label = labels, group = labels, hjust=1, angle = 90), vjust=0.5, hjust=1.1, na.rm=TRUE, size = geomText.size * 0.8, angle = 90, lineheight = 0.8) +
  geom_text(data=plotData %>% filter(period == 2040), aes(x=xmid , y = 2045, label = pct), vjust=0.5, hjust=-0.1, na.rm=TRUE, size = geomText.size, angle = 90) +
  scale_fill_manual(values = c(setNames(plotColor[names(vars)],vars)), labels = setNames(names(vars), vars)) + 
  scale_alpha_manual(values = c(0.2, 0.3, 1)) +
  scale_color_manual(values = c(setNames(plotColor[names(vars)],vars)), labels = setNames(names(vars), vars)) +
  ...
  coord_cartesian(clip="off") +
  theme(plot.margin = margin(t = 30, r = 0, b = 100, l = 0, unit = "pt")) +
  #labs(title = "(c) Sectoral breakdown of net CO2 emissions from energy and industrial processes (% of average 2018-2022)") + 
  #theme(plot.title = element_text(margin=margin(t=0,r=0,b=45,l=0)))
  labs(title = "(c) Sectoral breakdown of net CO2 emissions from energy and industrial processes (%)") + 
  theme(plot.title = element_text(margin=margin(t=0,r=0,b=45,l=0), hjust=1))
...
g[["Figure 1 - net"]] <- gridExtra::arrangeGrob(
  g[["GHG emi"]],
  g[["target"]],
  g[["2040 Net Emissions - Energy and Industrial Processes - horizontal bar"]],
  layout_matrix = rbind(c(1, 1, 1, 1, 2, 2),
                        c(1, 1, 1, 1, 2, 2),
                        c(1, 1, 1, 1, 2, 2),
                        c(1, 1, 1, 1, 2, 2),
...
ggsave(paste0(outChartsFolder,"/png/Figure 1. Emission results panel (net).png"), g[["Figure 1 - net"]], device="png", width = 12, height = 8, dpi=300, units = "in")
ggsave(paste0(outChartsFolder,"/svg/Figure 1. Emission results panel (net).svg"), g[["Figure 1 - net"]], device="svg", width = 12, height = 8, dpi=100, units = "in")
...
#grid.newpage()
#grid.draw(g[["Figure 1 - net"]])

```

<!-- Figure 1. Greenhouse gas emission reductions for scenarios reaching climate neutrality by 2050. Central values are given for the reference scenario (57% reduction by 2030, reference final energy projections, biomass availability limited to 7.5 EJ/yr and default CCS assumption), ranges for the full scenario ensemble. Pattern in net emissions represents carbon captured in the energy sector. -->


```{r, echo=FALSE, include=FALSE}

g[["Figure 1 - net only"]] <- gridExtra::arrangeGrob(
  g[["GHG emi"]],
  g[["target"]],
...
ggsave(paste0(outChartsFolder,"/png/Figure 1. Emission results panel (net only).png"), g[["Figure 1 - net only"]], device="png", width = 12, height = 8, dpi=300, units = "in")
ggsave(paste0(outChartsFolder,"/svg/Figure 1. Emission results panel (net only).svg"), g[["Figure 1 - net only"]], device="svg", width = 12, height = 8, dpi=100, units = "in")
...
g[["Electricity Generation"]] <- 
  ...
  geom_text(data=plotData2040[plotData2040$percentage >= 0.02,],aes(label = label, y=cumPercentage-percentage/2, x = xPos), vjust=0.5, hjust=0, na.rm=TRUE, color = "black", size=geomText.size*0.8) +
...
grid.newpage()
grid.draw(g[["milestone - violin targets"]])
...
g[["Carbon Price"]] <- ggplot(plotData,aes(x=period,y=value,group = scenario)) +
  ...
  labs(title = "(a) Carbon price (€/t CO2)") #+ 
  #theme(plot.title = element_text(hjust=-0.1))
...
<!-- (b) Energy supply electricity investments (billion €/yr) -->
...
g[["Energy Supply Investments"]] <- ggplot(plotData, aes(x=period,y=value, fill=variable)) +
  ...
  labs(title = "(b) Energy supply investments (billion €/yr) ") #+ 
...
plotData <- df %>% 
  filter(region %in% c("EU27","DEU","FRA"), period >= 2020, period <= 2050, scenario  == mainScen, variable %in% unlist(vars)) %>% 
